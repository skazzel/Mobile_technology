...
class TodosFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val dao = TodosDatabase.getInstance(application).todosQueries
        ...
        val adapter = TodoItemAdapter {todoId -> viewModel.onTodoItemClicked(todoId) }
        binding.viewModel = viewModel
        binding.todosList.adapter = adapter
        binding.lifecycleOwner = viewLifecycleOwner

        viewModel.todos.observe(viewLifecycleOwner) {
            todosList -> todosList?.let {
                adapter.data = it
            }
        }

        viewModel.navigateToTodo.observe(viewLifecycleOwner) {
            todoId ->
            todoId?.let {
                val action = TodosFragmentDirections.actionTodosFragmentToEditTodoFragment(todoId)
                this.findNavController().navigate(action)
                viewModel.onTodoVavigated()
            }
        }
